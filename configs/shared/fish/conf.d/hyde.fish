...
function bind_M_n_history
    for i in (seq 1 9)
        bind \e$i "for n in (seq 1 $i); history-search-backward; end; commandline -f repaint"
    end
end
bind_M_n_history
...
alias g='git'
# alias fastfetch='fastfetch --logo-type kitty'  # Disabled - using Karamel logo from config
