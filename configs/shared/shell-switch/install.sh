...
#######################################
detect_aur_helper() {
    info "Checking for AUR helper..." >&2

    if command -v yay &>/dev/null; then
        success "Found AUR helper: yay" >&2
        echo "yay"
        return 0
    elif command -v paru &>/dev/null; then
        success "Found AUR helper: paru" >&2
        echo "paru"
        return 0
    else
        error "No AUR helper found (yay or paru)" >&2
        error "Please install an AUR helper and run this script again" >&2
        error "" >&2
        error "Install yay: https://github.com/Jguer/yay#installation" >&2
        error "Install paru: https://github.com/Morganamilo/paru#installation" >&2
        return 1
    fi
}
