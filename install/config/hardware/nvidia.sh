# ==============================================================================
# Niri NVIDIA Setup Script for Arch Linux (Karamel)
# ==============================================================================
# This script automates the installation and configuration of NVIDIA drivers
# for use with Niri Wayland compositor on Arch Linux.
#
# Original Author: https://github.com/Kn0ax
# Adapted for Karamel/Niri
#
# ==============================================================================
...
if [ -n "$(lspci | grep -i 'nvidia')" ]; then
  ...
  # Add NVIDIA environment variables to niri/config.kdl
  NIRI_CONF="$HOME/.config/niri/config.kdl"
  if [ -f "$NIRI_CONF" ]; then
    # Check if NVIDIA env vars already exist
    if ! grep -q "NVD_BACKEND" "$NIRI_CONF"; then
      cat >>"$NIRI_CONF" <<'EOF'
