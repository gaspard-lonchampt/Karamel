# Directs user to Karamel Discord
QR_CODE='
█▀▀▀▀▀█ ▄ ▄ ▀▄▄▄█ █▀▀▀▀▀█
█ ███ █ ▄▄▄▄▀▄▀▄▀ █ ███ █
█ ▀▀▀ █ ▄█  ▄█▄▄▀ █ ▀▀▀ █
...
show_log_tail() {
  if [[ -f $KARAMEL_INSTALL_LOG_FILE ]]; then
    local log_lines=$(($TERM_HEIGHT - $LOGO_HEIGHT - 35))
    local max_line_width=$((LOGO_WIDTH - 4))

    tail -n $log_lines "$KARAMEL_INSTALL_LOG_FILE" | while IFS= read -r line; do
...
catch_errors() {
  ...
  gum style --foreground 1 --padding "1 0 1 $PADDING_LEFT" "Karamel installation stopped!"
  show_log_tail
  ...
  while true; do
    ...
    case "$choice" in
    "Retry installation")
      bash ~/.local/share/karamel/install.sh
      break
      ;;
    "View full log")
      if command -v less &>/dev/null; then
        less "$KARAMEL_INSTALL_LOG_FILE"
      else
        tail "$KARAMEL_INSTALL_LOG_FILE"
      fi
      ;;
    "Upload log for support")
      karamel-upload-install-log
      ;;
    "Exit" | "")
      exit 1
      ;;
