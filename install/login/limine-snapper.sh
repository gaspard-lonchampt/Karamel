if command -v limine &>/dev/null; then
  ...
  sudo tee /etc/mkinitcpio.conf.d/karamel_hooks.conf <<EOF >/dev/null
...
#timeout: 3
default_entry: 2
interface_branding: Karamel Bootloader
...
EOF
  ...
  if [[ -z ${KARAMEL_CHROOT_INSTALL:-} ]]; then
...
#
#   uki_file=$(find /boot/EFI/Linux/ -name "karamel*.efi" -printf "%f\n" 2>/dev/null | head -1)
#
#   if [[ -n "$uki_file" ]]; then
